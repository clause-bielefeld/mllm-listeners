...
class Qwen2_5(MLLMWrapper):
    def __init__(self, model_id="Qwen/Qwen2.5-VL-3B-Instruct", quant=None, cache_dir=None, **kwargs):
        """
        Initializes the Qwen2.5-VL model for vision-language generation.
        """
        import torch
    ...
    def generate(self,image, prompt, max_new_tokens=500, prune_output_to_response=False):
        ...
        # Create chat template text for the model
        text = self.processor.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=True
        )

        # Update processor call
        ...
        # Potential post-processing if desired
